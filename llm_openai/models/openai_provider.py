...
# chat.completions con herramientas Odoo; para búsqueda hospedada por OpenAI habría que
# integrar responses.create o un modelo *-search-preview.
...
class LLMProvider(models.Model):
    ...
    def openai_format_tools(self, tools):
        ...
        for tool, formatted in zip(tools or [], out):
            if tool.name != "odoo_record_creator":
                continue
            try:
                params = (formatted or {}).get("function", {}).get("parameters", {})
                wv = (params.get("properties") or {}).get("write_values")
                _logger.info(
                    "OpenAI: herramienta odoo_record_creator; write_values en esquema=%s",
                    json.dumps(wv, ensure_ascii=False) if wv is not None else None,
                )
            except (TypeError, ValueError) as err:
                _logger.info(
                    "OpenAI: no se pudo registrar esquema odoo_record_creator: %s", err
                )
        return out
    ...
    def openai_format_messages(self, messages, system_prompt=None):
        ...
        for message in messages:
            if isinstance(message, dict) and message.get("role"):
                formatted_messages.append(dict(message))
                continue
